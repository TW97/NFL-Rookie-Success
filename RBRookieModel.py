...
rb_model = sm.formula.glm("points ~ (age + pick + bmi + arm_length \
+ forty_yard + bench_press + burst_score + rush_td + rec_yards \
+ rush_att + rush_yards)", family=sm.families.Poisson(), data=rb_data).fit()

print(rb_model.summary())

print("Mean Squared Error: ", mse(rb_model.predict(rb_data), rb_data.points))

# 2019 Predictions... seems low for Jacobs?
rb_19 = master_data[(master_data.draft_year == 2019) & (master_data.position == 'RB')].copy()

rb_19['predicted_points'] = rb_model.predict(rb_19)


rb_19 = rb_19[['name', 'predicted_points', 'draft_year', 'age', 'round', 'pick',
'bench_press', 'bmi', 'arm_length', 'forty_yard', 'bench_press', 'burst_score', 
'rush_att', 'rush_yards', 'rush_td', 'rec', 'rec_yards', 'rec_td']]

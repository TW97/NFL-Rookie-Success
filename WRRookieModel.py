...
X = wr_data[['age', 'pick', 'height', 'arm_length', 'hand_size',
'height_adj_ss', 'games', 'rec', 'rec_yards', 'rec_td', 'avg_diff']]
...
for season in seasons:
    X_train = wr_data[wr_data.draft_year != season][['age', 'pick', 'height', 'arm_length', 'hand_size',
'height_adj_ss', 'games', 'rec', 'rec_yards', 'rec_td', 'avg_diff']]
    X_test = wr_data[wr_data.draft_year == season][['age', 'pick', 'height', 'arm_length', 'hand_size',
'height_adj_ss', 'games', 'rec', 'rec_yards', 'rec_td', 'avg_diff']]
    y_train = wr_data[wr_data.draft_year != season]['points']
    y_test = wr_data[wr_data.draft_year == season]['points']
...
# 2019 Predictions... seems to be lots of missing players ? DK, Deebo, Renfrow
wr_19 = master_data[(master_data.draft_year == 2019) & (master_data.position == 'WR')]
wr_19_x = wr_19[['age', 'pick', 'height', 'arm_length', 'hand_size',
'height_adj_ss', 'games', 'rec', 'rec_yards', 'rec_td', 'avg_diff']]

wr_model = ElasticNet(alpha=0.01)
wr_model.fit(X, y)
wr_predicted = wr_model.predict(wr_19_x)
print(pd.DataFrame(wr_model.coef_, wr_19_x.columns, columns=['Coefficient'])) 
print("Model Intercept: ", wr_model.intercept_) 
wr_19['predicted_points'] = wr_predicted

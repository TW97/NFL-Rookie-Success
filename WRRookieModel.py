...
wr_model = sm.formula.glm("points ~ (age + pick + bmi \
+ burst_score + hand_size + twenty_yard + forty_yard + rec_yards + rec_td)",
    family=sm.families.Poisson(), data=wr_data).fit()

print(wr_model.summary())

print("Mean Squared Error: ", mse(wr_model.predict(wr_data), wr_data.points))

# 2019 Predictions... seems to be lots of missing players ? DK, Deebo, Renfrow
wr_19 = master_data[(master_data.draft_year == 2019) & (master_data.position == 'WR')].copy()
wr_19['predicted_points'] = wr_model.predict(wr_19)
wr_19 = wr_19[['name', 'predicted_points', 'draft_year', 'age', 'round', 'pick',
'height', 'weight', 'arm_length', 'hand_size', 'twenty_yard', 'forty_yard',
